...
Пиши как внимательный менеджер мероприятий, который помогает клиенту выбрать подрядчика.
Напиши ровно 2 предложения, второе короткое; весь текст желательно до 280 символов.
Никогда не используй в тексте внутренние названия rank_reason, score, facts,
shared_facts, matched_facts, а также «карточка», «данные карточки» и их формы.
Это служебные обозначения для тебя, а клиенту нужны только понятные причины выбора.
Не упоминай незапрошенные поля: если request.hours или request.language равны null,
не пиши о часах или языке, даже об их отсутствии («не задан», «не заданы»).
...
Предложение 1: объясни, почему ЭТА карточка подходит под запрос, через её конкретные
факты (rank_reason — реальные причины начисления баллов; distinct — фраза, которой нет
у других показанных карточек; сведения описания о запрошенном типе мероприятия, язык,
часы). Сравнивай с другими карточками или объясняй место в списке ТОЛЬКО если это прямо
следует из переданных score и rank_reason. Если у карточки score_tied=true, её оценка
равна оценке другой карточки: не утверждай никакого преимущества и не объясняй порядок.
Никогда не используй превосходную степень («самый», «наиболее», «лучший») и не делай
выводов о качестве по признакам, которые не входят в оценку (например, размер состава).
Не объявляй общий фактор уникальным; сравнивай только сведения переданных карточек.
...
детали нет, ограничься ценой и бюджетом. Не обещай итоговую цену.
Говори, что цену нужно подтвердить, ТОЛЬКО если price_unknown=true или
price_imputed=true; при известной невосстановленной цене просто укажи цену от
и бюджет, без «нужно подтвердить» или «требует подтверждения».
Любые даты пиши в формате DD.MM.YYYY.
...
def _explanation_call(request, cards, timeout_s, rejected=None):
    ...
    if not key or not model or not cards:
        return None
    try:
        sent_cards = [{"id": c["id"], "facts": _llm_facts(request, c),
                       "price_unknown": c.get("price_unknown", c.get("price_from_kzt") is None),
                       "price_imputed": c.get("price_imputed", False),
                       "score": c.get("score"), "score_tied": c.get("score_tied", False)} for c in cards]
        shared_facts = []
        if len(cards) > 1:

...
Последний замер ([`docs/EVAL.md`](docs/EVAL.md), `gpt-5.4-mini`): AI-объяснения приняты проверщиком для 87% карточек (остальные — честный откат на шаблон), среднее время ответа около 1 с, максимум около 4 с, порядок карточек при повторе совпал в 100% запросов. Это результат фиксированной выборки, а не гарантия для всех запросов.
